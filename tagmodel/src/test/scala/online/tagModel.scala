...
object tagModel {
  ...
  private val LOG = LoggerFactory.getLogger("rf")
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val (_, p) = tool.read_convert(modelid, "P_SOURCE", sc, attributeInfo)
      LOG.info("P_SOURCE数量:" + p.count().toString)

      tool.log(modelid, "读取训练数据", "1", prop.getProperty("log"))
      val (_, u) = tool.read_convert(modelid, "U_SOURCE", sc, attributeInfo)
      LOG.info("U_SOURCE数量:" + u.count().toString)
      LOG.info("数据读取完成")

      //train model use p and u,return arg c and model
      val (estC, model) = fit.fit(modelid, p, u, algo_args, nominalInfo.toMap)
